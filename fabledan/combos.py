...
class Move:
    __slots__ = ("type", "key", "cards", "claim_ranks", "size", "claim_suit", "wildcards")

    def __init__(self, mtype, key, cards, claim_ranks, claim_suit=None, wildcards=()):
        self.type = mtype
...
def claim_ids(move):
    ...
    out = []
    force_suit = None
    if move.type == SFLUSH and move.claim_suit is not None:
        force_suit = move.claim_suit
    elif move.type == SFLUSH:
        ...
        if force_suit is None:
            force_suit = 0
    for c, r in zip(move.cards, move.claim_ranks):
        if c not in move.wildcards and rank_of(c) == r and (force_suit is None or suit_of(c) == force_suit):
            out.append(c)
            used.add(c)
        else:
...
def gen_moves_reduced(cards, lv, lead=None):
...
def gen_moves(cards, lv, lead=None):
    """Suit-aware legal actions. Local extension; see THIRD_PARTY.md."""
    from .legal import enumerate_moves
    return enumerate_moves(cards, lv, lead)
