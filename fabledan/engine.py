...
def default_return_card(cards, lv):
    ...
    cand = [c for c in cards
            if 1 <= rank_of(c) <= 9 and not is_wildcard(c, lv)]  # 2..10; A is high
    if not cand:
...
class GuandanRound:
    ...
    def __init__(self, level, rng=None, tribute_mode=None, deal=None):
        if not isinstance(level, int) or not 0 <= level < 13:
            raise ValueError("level must be a rank index from 0 to 12")
        self.lv = level
        ...
        if deal is None:
            ...
            flat = [c for hand in deal for c in hand]
            if len(deal) != 4 or len(flat) != len(set(flat)) or any(c not in range(108) for c in flat):
                raise ValueError("deal requires four hands with unique valid card IDs")
            if any(not hand for hand in deal):
                raise ValueError("all four hands must initially be nonempty")
    ...
    def play_steps(self):
        ...
        while len(self.done_order) < 3:
            ...
            if len(legal) == 1:
                ...
                idx = yield obs
                if not isinstance(idx, int) or isinstance(idx, bool) or not 0 <= idx < len(legal):
