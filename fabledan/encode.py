...
# Local v2 encoding: exact face counts and public played-card inventory.
# Suits matter for preserving flushes. Claimed ranks alone lose wildcard use.
FEAT_DIM = BASE_FEAT_DIM + 54 * 3 + 4 + 2
ENCODING_VERSION = "guandan-lab-v2"
    ...
    for ev in obs["events"]:
        if ev[0] == 'play':
            for c in ev[2].cards:
                f[i + c % 54] += 0.5
    i += 54
    owner = obs.get("lead_owner")
    if owner is not None:
        f[i + (owner - me) % 4] = 1
    i += 4
    f[i] = len(obs["events"]) / 256.0
    f[i + 1] = 1.0 if owner == (me + 2) % 4 else 0.0
    i += 2
    assert i == FEAT_DIM
    return f
