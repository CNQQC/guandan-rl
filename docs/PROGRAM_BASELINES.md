...
本次实际从 640 局检查点续训了 240 局，保存为 `runs/league-first`：南邮 36 局、蛋饼 37 局、历史模型 83 局、本地规则 35 局、完全自我对弈 49 局。累计 880 局、50,135 条样本。该次独立目录续训保留模型、优化器与回放，并以原检查点重新建立历史对手池。

随后在独立种子的各 100 局测试中，学习模型对南邮胜率 **1%**，对蛋饼 **4%**。这说明它确实在和更强程序博弈，也说明目前仍处于冷启动阶段，**尚未学到能够稳定击败程序基线的策略**。不能把网络更新次数、训练吞吐量或公开 DanLM 的成绩当作这个模型已经变强的证据。
